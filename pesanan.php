    ...
    try {
        ...
        if ($id > 0) {
            ...
            if ($deleted && $stmt->rowCount() > 0) {
                ...
                header('Location: kelola-pesanan.php' . ($q ? '?q=' . urlencode($q) : ''));
                exit;
            }
        }
...
try {
    $sql = "SELECT id, nama_pelanggan, alamat, produk, total, pengambilan, metode_bayar, status, created_at
            FROM pesanan WHERE $where ORDER BY created_at DESC";
...
<div class="container">
    <div class="stats">
        <div class="stat-card">
            <div class="stat-value"><?= count(array_filter($orders, fn($o) => $o['status'] === 'baru')) ?></div>
            <div class="stat-label">Baru</div>
        </div>
        <div class="stat-card">
            <div class="stat-value"><?= count(array_filter($orders, fn($o) => $o['status'] === 'diproses')) ?></div>
            <div class="stat-label">Diproses</div>
        </div>
        <div class="stat-card">
            <div class="stat-value"><?= count(array_filter($orders, fn($o) => $o['status'] === 'selesai')) ?></div>
            <div class="stat-label">Selesai</div>
        </div>
        <div class="stat-card">
            <div class="stat-value"><?= count(array_filter($orders, fn($o) => $o['status'] === 'batal')) ?></div>
            ...
            <?php foreach ($orders as $o): ?>
                <div class="order-card">
                    ...
                    <div class="order-footer">
                        ...
                        <a href="?hapus=<?= (int)$o['id'] ?><?= $q ? '&q=' . urlencode($q) : '' ?>"
